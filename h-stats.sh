...
if [ "$diffTime" -lt "$maxDelay" ]; then
  ...
  if [ -z "$cpu_count" ] && tail -n 1 "$log_head_name" | grep -q "Alias .*xCPU"; then
    cpu_count=1
  else
    cpu_count=0
  fi
  ...
  if [ $cpu_count -eq 0 ] || [ $gpu_count -eq 0 ]; then
    echo "..."
    cat $log_name | grep -E "threads are used|CUDA devices are used|Alias .*xCPU" | tail -n 100 > $log_head_name
